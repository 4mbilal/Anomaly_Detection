#!/usr/bin/env bash
# ============================================================
#  GLASS baseline: toothbrush only, full paper settings (Colab/Linux).
#  --distribution 2 forces the MANIFOLD hypothesis (toothbrush's shipped
#  baseline value), so it needs no distribution xlsx lookup.
#  EDIT the two paths below to your extracted /content locations.
#  Do NOT change --resize / --imagesize: toothbrush uses a special
#  329/288 resize ratio that only matches the paper at imagesize 288.
#  fg_mask must exist at $datapath/fg_mask/toothbrush/*.png (because --fg 1).
# ============================================================
datapath=/content/mvtec_anomaly_detection
augpath=/content/dtd/images

python main.py \
    --gpu 0 \
    --seed 0 \
    --test ckpt \
  net \
    -b wideresnet50 \
    -le layer2 \
    -le layer3 \
    --pretrain_embed_dimension 1536 \
    --target_embed_dimension 1536 \
    --patchsize 3 \
    --meta_epochs 640 \
    --eval_epochs 1 \
    --dsc_layers 2 \
    --dsc_hidden 1024 \
    --pre_proj 1 \
    --mining 1 \
    --noise 0.015 \
    --radius 0.75 \
    --p 0.5 \
    --step 20 \
    --limit 392 \
  dataset \
    --distribution 2 \
    --mean 0.5 \
    --std 0.1 \
    --fg 1 \
    --rand_aug 1 \
    --batch_size 8 \
    --num_workers 4 \
    --resize 288 \
    --imagesize 288 \
    -d toothbrush \
    mvtec $datapath $augpath
