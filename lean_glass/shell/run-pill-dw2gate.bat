@echo off
REM Additional dataset: pill. Image-level metrics only (no GT masks).
REM Locked model: MobileNetV2 dw2 + gate, 100 epochs, fg=0, forced distribution 3.
set datapath=C:\data\anomaly_extra
set augpath=C:\data\dtd\images

python main.py ^
    --gpu 0 --seed 0 --test ckpt --run_name pill_dw2gate_100ep ^
  net ^
