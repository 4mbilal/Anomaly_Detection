...
class StreamGate(torch.nn.Module):
    """Learnable per-stream weighting applied before the Aggregator's average.

    Input:  (N, n_streams, dim)  (the stack from Preprocessing)
    Output: (N, n_streams, dim)  scaled per stream.

    weights = softmax(logits) * n_streams, with logits initialised to 0, so at
    init every stream weight is exactly 1 and the module reproduces the
    unweighted average. Training can then up/down-weight streams; because GLASS
    trains one model per class, the weighting is learned per class. This breaks
    the equal-weight averaging that let an uninformative stream degrade results.
    """
    def __init__(self, n_streams):
        super(StreamGate, self).__init__()
        self.n_streams = n_streams
        self.logits = torch.nn.Parameter(torch.zeros(n_streams))

    def forward(self, features):
        w = torch.softmax(self.logits, dim=0) * self.n_streams  # (n_streams,)
        return features * w.view(1, -1, 1)
